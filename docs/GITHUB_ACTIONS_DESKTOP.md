...
Windows 打包在 MSYS2 的 MINGW64 环境中编译。构建脚本会从源码编译 cmark-gfm、运行测试、递归收集 MinGW DLL，并额外打包 GTK 图像加载器、GSettings、Adwaita 图标与 GIO TLS 模块。下载后应解压并通过 `bin/filecollector-launch.bat` 启动，以正确设置图片加载器与 HTTPS 模块路径。[3]

macOS 打包固定在 Apple Silicon 的 `macos-14` 运行器上，产出 ARM64 `.app` 并将 Homebrew 动态库重定位到应用包内。产物使用临时的 ad-hoc 签名用于完整性验证，但**不含 Apple Developer ID 签名或公证**；首次在其他 Mac 上打开时，系统仍可能显示未验证开发者提示。[4]
