    ...
    public static Gee.ArrayList<ResolvedItem> resolve_items (
        ...
        foreach (var data in items) {
            ...
            if (!f.query_exists () || data.is_missing) {
                ...
                ri.error_message = "[缺失文件: %s]".printf (data.file_path);
                result.add (ri);
                continue;
            }
            ...
            try {
                ...
                ri.error_message = "[无法获取文件信息: %s]".printf (e.message);
                result.add (ri);
                continue;
            }
            if (file_size > MAX_FILE_CONTENT_SIZE) {
                ri.kind = ItemKind.TOO_LARGE;
                ri.error_message = "[文件过大 (%s), 已跳过内容读取]".printf (UIHelpers.format_size (file_size));
                result.add (ri);
                continue;
            }
            ...
            try {
                ...
                ri.kind = ItemKind.READ_ERROR;
                ri.error_message = "[读取文件失败: %s]".printf (e.message);
                result.add (ri);
            } finally {
                if (fis != null) {
